...
type NameItem = { name: string; meaning: string };
...
const names: Record<Style, Record<Gender, NameItem[]>> = {
  modern: {
    girl: [
      ...
      { name: "书瑶", meaning: "书代表知识，瑶代表美玉，寓意温润有才。" },
      { name: "亦然", meaning: "简洁自然，寓意真实、安定与从容。" },
    ],
    boy: [
      ...
      { name: "予泽", meaning: "予有给予之意，泽代表润泽与善意。" },
      { name: "明屿", meaning: "明代表清晰明朗，屿有独立沉稳之感。" },
    ],
    neutral: [
      ...
      { name: "星禾", meaning: "星象征希望，禾象征成长。" },
      { name: "予安", meaning: "予有给予之意，安代表安定平和。" },
    ],
  },
  elegant: {
    girl: [
      ...
      { name: "静姝", meaning: "静为沉静，姝为美好，整体文雅古典。" },
      { name: "令仪", meaning: "有美好仪度之意，寓意端庄大方。" },
    ],
    boy: [
      { name: "怀瑾", meaning: "取怀抱美玉之意，寓意品性温润。" },
      { name: "修远", meaning: "修代表修养，远代表志向长远。" },
      { name: "景和", meaning: "景有光明之意，和代表温良平和。" },
      { name: "承礼", meaning: "承有传承担当，礼代表修养与分寸。" },
      { name: "若衡", meaning: "若带文雅气质，衡代表稳重平衡。" },
    ...
    neutral: [
      ...
      { name: "景初", meaning: "景象光明，初代表初心。" },
      { name: "云知", meaning: "云有舒展意象，知代表智慧。" },
    ],
  },
  literary: {
    girl: [
      ...
      { name: "月禾", meaning: "月有柔光，禾寓意成长与丰盈。" },
      { name: "书棠", meaning: "书有文雅，棠象征美好与生机。" },
    ],
    boy: [
      ...
      { name: "望舒", meaning: "有月光意象，寓意温和明亮。" },
      { name: "景行", meaning: "寓意高尚品行与光明方向。" },
    ],
    neutral: [
      ...
      { name: "书远", meaning: "书代表知识，远代表格局。" },
      { name: "明澈", meaning: "寓意清澈通透。" },
    ],
  },
  bright: {
    girl: [
      ...
      { name: "晓禾", meaning: "晓是清晨，禾象征生长。" },
      { name: "予晴", meaning: "予代表给予，晴代表明亮。" },
    ],
    boy: [
      ...
      { name: "皓然", meaning: "皓有明亮洁白之意，然代表自然从容。" },
      { name: "星泽", meaning: "星象征希望，泽代表润泽善意。" },
    ],
    neutral: [
      ...
      { name: "景宁", meaning: "景为光明，宁为安宁。" },
      { name: "昭禾", meaning: "昭有明亮之意，禾代表成长。" },
    ],
  },
  nature: {
    girl: [
...
export default function ChineseNameGeneratorPage() {
  ...
  const suggestions = useMemo(() => getSuggestions(surname, gender, style), [surname, gender, style]);
      ...
      <section className="relative overflow-hidden bg-[#FAF8F5]">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-8 py-14 md:py-20">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-[#F0E8DD] bg-white/75 px-4 py-2 text-sm font-medium text-[#B8915D] shadow-sm backdrop-blur mb-5"><span>✦</span><span>妈妈工具</span></div>
            <h1 className="text-4xl md:text-5xl font-bold leading-tight text-[#1F4E4C] mb-5">宝宝中文起名工具</h1>
            <p className="max-w-xl text-base md:text-lg leading-8 text-gray-600">输入姓氏、宝宝性别和喜欢的名字风格，生成一组中文名建议和寓意说明。名字仅供灵感参考。</p>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-8">
          <div className="grid lg:grid-cols-[0.9fr_1.1fr] gap-8 lg:gap-10 items-start">
            <div className="rounded-3xl border border-[#F0E8DD] bg-[#FAF8F5] p-6 md:p-8 shadow-sm">
              ...
              <h2 className="text-3xl md:text-4xl font-bold text-[#1F4E4C] leading-tight mb-5">生成中文名灵感</h2>
              <form onSubmit={(e) => { e.preventDefault(); setGenerated(true); }} className="grid gap-5">
                <label className="grid gap-2"><span className="text-sm font-medium text-[#1F4E4C]">姓氏 *</span><input required value={surname} onChange={(e) => setSurname(e.target.value)} maxLength={2} placeholder="例如：李、王、陈" className="rounded-2xl border border-[#E8DCC9] bg-white px-4 py-3 text-gray-700 outline-none transition focus:border-[#D6B37F] focus:ring-2 focus:ring-[#D6B37F]/20" /></label>
                <label className="grid gap-2"><span className="text-sm font-medium text-[#1F4E4C]">宝宝性别</span><select value={gender} onChange={(e) => setGender(e.target.value as Gender)} className="rounded-2xl border border-[#E8DCC9] bg-white px-4 py-3 text-gray-700 outline-none transition focus:border-[#D6B37F] focus:ring-2 focus:ring-[#D6B37F]/20"><option value="girl">女孩</option><option value="boy">男孩</option><option value="neutral">中性 / 不限定</option></select></label>
                <label className="grid gap-2"><span className="text-sm font-medium text-[#1F4E4C]">名字风格</span><select value={style} onChange={(e) => setStyle(e.target.value as Style)} className="rounded-2xl border border-[#E8DCC9] bg-white px-4 py-3 text-gray-700 outline-none transition focus:border-[#D6B37F] focus:ring-2 focus:ring-[#D6B37F]/20"><option value="modern">现代简洁</option><option value="elegant">温润文雅</option><option value="literary">诗意书卷</option><option value="bright">阳光大气</option><option value="nature">自然清新</option></select></label>
                <button type="submit" className="rounded-full bg-[#1F4E4C] px-8 py-3.5 text-center font-semibold text-white transition hover:bg-[#173D3B]">生成名字建议</button>
              </form>
            </div>
            ...
            <div className="rounded-3xl border border-[#F0E8DD] bg-white p-6 md:p-8 shadow-sm">
              ...
              {!generated ? <div className="rounded-3xl bg-[#FAF8F5] p-6 md:p-8 text-gray-600 leading-8">请先输入姓氏并选择名字风格。点击“生成名字建议”后，这里会显示中文名组合和寓意说明。</div> : <div className="grid gap-4">{suggestions.map((item) => <div key={item.fullName} className="rounded-2xl border border-[#F0E8DD] bg-[#FAF8F5] p-5 transition hover:-translate-y-0.5 hover:shadow-md"><div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3"><div className="text-2xl font-bold text-[#1F4E4C]">{item.fullName}</div><span className="inline-flex w-fit rounded-full bg-white px-3 py-1 text-xs font-semibold text-[#B8915D] border border-[#F0E8DD]">{item.style}</span></div><p className="text-gray-600 leading-7">{item.meaning}</p></div>)}</div>}
            </div>
          </div>

          {generated && <ShareToolBox className="mt-8 md:mt-10" title="宝宝中文起名｜加华月子餐" text="这个宝宝中文起名工具可以根据姓氏、性别和名字风格生成中文名灵感，还会附带寓意说明。" heading="身边有正在给宝宝起名的朋友？" description="把这个中文起名工具发给她，可以快速获得一些好听、有寓意的中文名灵感。" shareText={`我发现一个宝宝中文起名工具，可以根据姓氏、宝宝性别和名字风格生成中文名灵感，还会附带寓意说明。适合正在给宝宝起名的家庭参考：\n\n${typeof window !== "undefined" ? window.location.href : ""}`} />}
        </div>
      </section>
